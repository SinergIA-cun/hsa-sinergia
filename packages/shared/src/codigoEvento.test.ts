...
describe('codigoEvento', () => {
  ...
  it('el año distingue: el mismo día y salón de dos años distintos NO se empalman', () => {
    ...
    expect(codigoEvento({ ...base, fechaISO: '2027-10-29' })).toBe('29OCT27-CBARRERA-CUPULA');
    expect(codigoEvento({ ...base, fechaISO: '2028-10-29' })).toBe('29OCT28-CBARRERA-CUPULA');
    expect(codigoEvento({ ...base, fechaISO: '2029-10-29' })).toBe('29OCT29-CBARRERA-CUPULA');
  });

  it('el año son los DOS últimos dígitos, con su cero al frente', () => {
    const base = { cliente: 'Ana Ruiz', espacios: ['Salón Los Arcos'] };
    expect(codigoEvento({ ...base, fechaISO: '2030-05-08' })).toBe('08MAY30-ARUIZ-ARCOS');
    expect(codigoEvento({ ...base, fechaISO: '2100-05-08' })).toBe('08MAY00-ARUIZ-ARCOS');
  });

  it('el día conserva sus dos dígitos y el mes es la abreviatura en español', () => {
    const base = { cliente: 'Ana Ruiz', espacios: ['Salón Los Arcos'] };
    expect(codigoEvento({ ...base, fechaISO: '2027-07-04' })).toBe('04JUL27-ARUIZ-ARCOS');
    expect(codigoEvento({ ...base, fechaISO: '2027-12-31' })).toBe('31DIC27-ARUIZ-ARCOS');
    expect(codigoEvento({ ...base, fechaISO: '2027-09-01' })).toBe('01SEP27-ARUIZ-ARCOS');
  });
  ...
  it('el espacio ignora artículos y preposiciones: manda la última palabra con contenido', () => {
    ...
    expect(codigoEvento({ ...base, espacios: ['Salón Los Arcos'] })).toBe('10ABR27-CBOLADO-ARCOS');
    expect(codigoEvento({ ...base, espacios: ['Jardín Los Campos'] })).toBe('10ABR27-CBOLADO-CAMPOS');
    expect(codigoEvento({ ...base, espacios: ['Salón Los Balcones'] })).toBe('10ABR27-CBOLADO-BALCONES');
    // Un nombre que TERMINA en artículo cae a la palabra anterior en vez de quedarse vacío.
    expect(codigoEvento({ ...base, espacios: ['Jardín La Cúpula de La'] })).toBe('10ABR27-CBOLADO-CUPULA');
  });
  ...
  it('sin cliente o sin espacio el código no se rompe: usa un relleno visible', () => {
    expect(codigoEvento({ fechaISO: '2027-11-13', cliente: '', espacios: [] })).toBe('13NOV27-NA-NA');
    expect(codigoEvento({ fechaISO: '2027-11-13', cliente: '***', espacios: ['###'] })).toBe('13NOV27-NA-NA');
  });
