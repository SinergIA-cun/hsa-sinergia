...
- Plazo del complemento: 3 meses después de contratar (informativo en el plan).
...
- `formalizada`: pagado ≥ anticipo + complementoPct × total
...
### E. Log de actividad (por cotización, visible por cliente)

Eventos registrados: `creada` · `estatus` · `pago` · `edicion`.
...
model Payment {
  ...
  quoteId        String
  monto          Int                         // pesos, igual que total/rentaTotal
  metodo         PaymentMethod               // efectivo|transferencia|tarjeta (enum ya existe)
  concepto       PaymentConcept              // anticipo|aCuenta|finiquito (enum ya existe)
...
```
enum ActivityType { creada  estatus  pago  edicion }
...
- **pagado** = Σ `Payment.monto` de la cotización
- **saldo** = `total − pagado`
- **plan de hitos** (si el espacio tiene `SpacePaymentRule`):
  1. **Apartar**: objetivo = `anticipo`
  2. **Formalizar (complemento)**: objetivo acumulado = `anticipo + complementoPct × total`; vence ~3 meses después de contratar
  3. **Finiquito**: objetivo = `total`; vence `liquidarDiasAntes` antes del evento
...
- `GET /api/c/:token` — estado de cuenta real (pagado/saldo/plan) + lista de pagos (sin datos internos de quién capturó).
- `GET /api/quotes/:id/comprobante/:paymentId` — proxy que transmite la imagen desde Drive (auth requerida; público con token para la página del cliente).
...
- **Lista de pagos** con su comprobante (miniatura vía proxy).
...
- `Payment`/`ActivityLog`/campos operativos: nuevos, sin datos históricos que migrar. Cotizaciones existentes arrancan con pagado=0 real (Σ sin pagos) — consistente.
